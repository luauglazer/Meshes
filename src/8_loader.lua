	do
		...
		if load.IsLoaded == false and not (AssetSource == "Exploit Workspace" or inExploitWorkspace or _gca ~= nil) then
			...
			if loadupFPerson > 0 then
				local hasBody = (loadupFPerson == 1 or loadupFPerson == 2 or loadupFPerson == 4 or loadupFPerson == 6)
				local isSnappy = (loadupFPerson == 3 or loadupFPerson == 4)
				local noHead = (loadupFPerson == 5 or loadupFPerson == 6)

				PlayerData[Player.Name].FPerson = (loadupFPerson ~= 2)
				PlayerData[Player.Name].FPsnap = isSnappy
				PlayerData[Player.Name].HeadTracking = not noHead
